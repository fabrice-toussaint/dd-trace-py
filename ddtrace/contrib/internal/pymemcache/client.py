...
config._add(
    "pymemcache",
    dict(
        _default_service=schematize_service_name(memcachedx.SERVICE),
...
class _WrapperBase(wrapt.ObjectProxy):
    ...
    def _trace_function_as_command(self, func, cmd, *args, **kwargs):
        ...
        return _trace(func, cmd, self._datadog_tags, *args, **kwargs)
...
def _trace(func, method_name, tags, *args, **kwargs):
    ...
    with tracer.trace(
        ...
        service = trace_utils.ext_service(None, config.pymemcache)
        set_service_and_source(span, service, config.pymemcache)
